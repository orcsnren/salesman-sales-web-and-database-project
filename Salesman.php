...
$des=$_POST["Markets"]; 
$result=mysql_query("SELECT SALESMAN.Name,SALESMAN.Surname,COUNT(PRODUCT.product_id) FROM PRODUCT LEFT JOIN SALE ON PRODUCT.product_id=SALE.product_id LEFT JOIN SALESMAN ON SALE.salesman_id=SALESMAN.salesman_id LEFT JOIN MARKET ON SALESMAN.market_id=MARKET.market_id WHERE MARKET.market_id='".$des."' GROUP BY SALESMAN.salesman_id ORDER BY COUNT(PRODUCT.product_id) DESC");
...
{	
			...
			array_push($surnames,$row[1]);
	        array_push($counter,$row[2]);
...
<div class="container">
<center>
  <h2>Salesmans Information for Selected Market!</h2>
  <br><br>
  ...
  <div class="progress">
    <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="<?php $i?>" aria-valuemin="0" aria-valuemax="100" style="width:<?php echo $percent[$i]?>%">
      <?php echo $names[$i]." ".$surnames[$i]." (".$counter[$i]." Products".")" ?>
    </div>
  </div>
  <?php } ?>
</center>
